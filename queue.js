...
(() => {
  ...
  /* 무료 판정 상수(FREE_PIXELS · FREE_STEPS)는 overlay.js 것을 NibUsage.paidSize 로 빌려 쓴다.
   * 같은 격리 world 이고 manifest 에서 overlay.js 가 먼저 실린다. page.js 에만 사본이 있다(MAIN world). */
  /* 장당 시간은 메가픽셀 단위로 잰다. 값이 무료 상한(1048576)과 같은 것은 우연이다 —
  ...
  /* Opus 예측. analyze 의 평균은 **기준 장(832×1216·28스텝) 한 장당**이다. 베이스의 크기·스텝을
   * 같은 환산식(NibUsage.unitsPerImage — overlay.js 의 A3·A4)으로 기준 장 수로 바꿔 곱한다.
   * 오차와 "몇 장 더"(toTarget)도 analyze 가 이미 들고 있다. */
  function opusEstimate(total) {
    ...
    if (!usesOpus()) return { ok: false, free: true, freeWhy: 'V4.5는 할당량을 쓰지 않습니다' };
    const p = st.base?.params || readParams();
    // Anlas 과금 크기는 할당량을 안 쓴다 — A1, 원장 실측 (overlay.js 참고).
    if (NibUsage.paidSize(p.width, p.height, p.steps)) {
      return { ok: false, free: true, freeWhy: 'Anlas 과금 크기 — 할당량을 쓰지 않습니다' };
    }
    const units = NibUsage.unitsPerImage(p.width, p.height, p.steps);
    let a;
    try {
    ...
    return {
      ...
      consumed: a.consumed,
      reliable: a.reliable,
      /* 오차 ±10%까지 이 베이스로 몇 장이 더 필요한가. analyze 는 기준 장으로 주므로 환산한다.
         낼 수 없으면 null — analyze 가 그렇게 준다. */
      toTarget: a.toTarget == null ? null : Math.ceil(a.toTarget / units),
   ...
   *  얼려둔 베이스만 보는 여기서는 알 수 없다. 즉 **V4.5에서 레퍼런스를 붙인 채 돌리면
   *  `무료 구간`이라고 적혀 있어도 실제로는 Anlas 가 나간다**(V5에는 레퍼런스 기능이 아직 없다).
   *  그래서 이 값은 표시용일 뿐이고, 실제 안전판은 잔액을 되읽는 guardStop 이다.
   *
   *  장수 조건(`요청당 1장`)은 따지지 않는다. 큐는 BATCH_MAX 가 1이라 언제나 충족된다.
   *
   *  overlay.js 가 안 실렸으면(NibUsage 없음) 판정을 포기하고 **과금 쪽으로** 적는다 —
   *  "무료"라고 적어 두고 돈이 나가는 것보다, 나갈 수 있다고 알리는 편이 안전하다.
   *  예전처럼 NibUsage 를 바로 부르면 그 경우 예약 창 전체가 그려지지 않는다. */
  const isFreeSize = (p) => !!globalThis.NibUsage && !NibUsage.paidSize(p.width, p.height, p.steps);
  ...
  const CSS =
    ...
    .btn:not(:disabled):active { transform: scale(.94); }
  ...
  function renderTotals(check) {
    ...
    const opusV = o.free
      ? `<span class="v good">0%</span><span class="s">${esc(o.freeWhy)}</span>`
      : !o.ok
        ? `<span class="v weak">—</span><span class="s">${esc(o.why)}</span>`
        : `<span class="v">−${o.drop.toFixed(1)}%</span><span class="s">${o.relative < 0.01 ? '±1% 미만' : '±' + Math.round(o.relative * 100) + '%'}${
    ...
    if (o.ok && !o.reliable) {
      ...
      notes += `<div class="note"><span class="t">표본</span><div>Opus 예측은 누적 소모 <b>${o.consumed.toFixed(
        2
