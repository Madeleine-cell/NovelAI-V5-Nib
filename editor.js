...
(() => {
  ...
  const CSS = NibUI.shellCSS('612px') + `
    ...
    input, select, textarea {
      ...
      /* outline: none 대신 투명한 링. 강제 색 모드에서는 브라우저가 이걸 시스템 색으로 칠한다. */
    ...
    /* --accent-line 은 흰 바탕에서 1.48:1 이다. 포커스 표시는 --accent 로 그려야
       비문자 대비 3:1 하한을 넘는다. */
    ...
    .emoji-btn {
      ...
      border: 1px solid var(--hairline); border-radius: var(--r-ctl);
      cursor: pointer; transition: background-color .18s var(--ease), border-color .18s var(--ease), transform .18s var(--ease);
    }
    ...
    .seg button {
      ...
      transition: background-color .16s var(--ease), border-color .16s var(--ease), color .16s var(--ease), box-shadow .16s var(--ease), transform .16s var(--ease);
    }
    ...
    /* 누르는 느낌은 창 안 모든 버튼에 같은 두 단계로 (사이드 패널과 같은 규칙) */
    .btn:active, .emoji-btn:active, .seg button:active,
    .bookmarks button:active, .slot-remove:active { transform: scale(.94); }
    .slot-add:active { transform: scale(.985); }
    @media (prefers-reduced-motion: reduce) {
      .btn:active, .emoji-btn:active, .seg button:active,
      .bookmarks button:active, .slot-remove:active, .slot-add:active { transform: none; }
    }
