...
function makeCard(item) {
  ...
  // 카드 폭을 넘는 이름은 말줄임표로 잘린다. 전체를 볼 방법이 하나는 있어야 한다.
...
async function initTheme() {
  ...
  applyTheme(mode);
  // 여기까지 와야 테마가 확정된다. body 전환은 이 표식 뒤에서만 걸리므로
  // 저장소를 기다리는 동안 칠해진 오답이 페이드로 **보이지** 않는다.
...
/* 설정 창은 showModal()로 열려 top layer 에 있다. z-index 로는 그 위에 못 뜨므로
 * 토스트도 popover 로 같은 층에 올린다. 이걸 안 하면 창 **안에서** 띄운 안내가
 * 창 뒤에 가려 아무에게도 보이지 않는다.
 *
 * showPopover()는 이미 열려 있을 때 부르면 던진다. 열려 있으면 글자만 갈고 시계만 다시 잰다 —
 * 다시 닫았다 열면 눈에 띄는 깜빡임이 생긴다. */
let toastTimer;
function toast(msg, isError = false) {
  ...
  // popover 를 못 쓰는 환경 — 예전 방식으로 떨어진다 (설정 창 뒤에 가리는 것은 감수).
