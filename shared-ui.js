...
globalThis.NibUI = (() => {
  ...
  const TOKENS_LIGHT = `
    ...
    --glow-accent: 0 2px 10px hsl(38 54% 37% / 0.28);
    --glow-accent-s: 0 2px 8px hsl(38 54% 37% / 0.22);
  `;
  ...
  const TOKENS_DARK = `
    ...
    --glow-accent: 0 2px 10px hsl(245 50% 2% / 0.5);
    --glow-accent-s: 0 2px 8px hsl(245 50% 2% / 0.4);
  `;
  ...
  function shellCSS(winWidth = '592px') {
    ...
    *::-webkit-scrollbar-thumb:hover { background: var(--ink-3); background-clip: padding-box; }
    ...
    /* 동작을 줄여 달라고 한 사용자에게는 전환을 끈다.
       시간만 0으로 만들면 눌림·호버로 생기는 변형이 "순간이동"으로 남으므로
       transform 은 따로 꺼야 한다. 나타남·사라짐(opacity)은 그대로 둔다 —
       끄면 무엇이 열렸는지 알 수 없게 된다. */
